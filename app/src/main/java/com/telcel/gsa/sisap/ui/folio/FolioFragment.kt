...
class FolioFragment : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View {
        ...
        adapter = FoliosAdapter(FoliosAdapter.FolioListener { idFolio ->
            folioViewModel.onFolioClicked(idFolio)
        })
    ...
    private fun createChipsFilters(binding: FragmentFolioBinding, statusFilterList: List<String>?) {
        ...
        statusFilterList?.forEach { it ->
            ...
            chip.chipBackgroundColor =
                ColorStateList.valueOf(requireContext().getColor(R.color.color_primary))
            chip.setTextColor(requireContext().getColor(R.color.color_on_primary))
            chip.checkedIconTint =
                ColorStateList.valueOf(requireContext().getColor(R.color.color_secondary_variant))
